...
public class MmsSmsProvider extends ContentProvider {
    ...
    /*Begin: Modified by vicent for MSG_TRADICTION 2012/06/09*/
    private static final int URI_TRADICTIONBOX                     = 19;
    /*End: Modified by vicent for MSG_TRADICTION 2012/06/09*/
    /*Begin: Modified by zhongji for AHONG-ADD-CODE-FOR-PICK_CONTACT-EV23 2012/06/26*/
    private static final int URI_MESSAGE_BY_BOX                    = 20;
    /*End: Modified by zhongji for AHONG-ADD-CODE-FOR-PICK_CONTACT-EV23 2012/06/26*/
    ...
    /*Begin: Modified by vicent for MSG_NETWORKTYPE 2012/06/08*/
    ...
    private static final String[] MMS_SMS_COLUMNS =
        { BaseColumns._ID, Mms.DATE, Mms.DATE_SENT, Mms.READ, Mms.THREAD_ID,
          Mms.LOCKED, Mms.SEEN, Mms.SUB_ID, Mms.NETWORKTYPE_ID };
    /*End: Modified by vicent for MSG_NETWORKTYPE 2012/06/08*/
    ...
    /*Begin: Modified by vicent for MSG_TRADICTION 2012/06/12*/
    private static final String MMS_TRADICTION_CONSTRAINT = "(" +
            Mms.MESSAGE_TYPE + " = " + PduHeaders.MESSAGE_TYPE_SEND_REQ + " OR " +
            Mms.MESSAGE_TYPE + " = " + PduHeaders.MESSAGE_TYPE_RETRIEVE_CONF + " OR " +
            Mms.MESSAGE_TYPE + " = " + PduHeaders.MESSAGE_TYPE_NOTIFICATION_IND + ")";
    /*End: Modified by vicent for MSG_TRADICTION 2012/06/12*/
    ...
    static {
        ...
        /*Begin: Modified by vicent for MSG_TRADICTION 2012/06/09*/
        URI_MATCHER.addURI(
                AUTHORITY, "tradictionbox/#", URI_TRADICTIONBOX);
        /*End: Modified by vicent for MSG_TRADICTION 2012/06/09*/

        /*Begin: Modified by zhongji for AHONG-ADD-CODE-FOR-PICK_CONTACT-EV23 2012/06/26*/
        URI_MATCHER.addURI(AUTHORITY, "message/box", URI_MESSAGE_BY_BOX);
        /*End: Modified by zhongji for AHONG-ADD-CODE-FOR-PICK_CONTACT-EV23 2012/06/26*/
    ...
    public Cursor query(Uri uri, String[] projection,
        ...
        switch(URI_MATCHER.match(uri)) {
            ...
            case URI_CONVERSATIONS_MESSAGES:
                ...
                break;
            /*Begin: Modified by vicent for MSG_TRADICTION 2012/06/09*/
            case URI_TRADICTIONBOX:
                cursor = getTradictionboxMessages(uri.getPathSegments().get(1), projection,
                        selection, sortOrder);
                break;
            /*End: Modified by vicent for MSG_TRADICTION 2012/06/09*/
            /*Begin: Modified by zhongji for AHONG-ADD-CODE-FOR-PICK_CONTACT-EV23 2012/06/26*/
            case URI_MESSAGE_BY_BOX:
                cursor = getboxMessages(projection, selection, sortOrder);
                break;
            /*End: Modified by zhongji for AHONG-ADD-CODE-FOR-PICK_CONTACT-EV23 2012/06/26*/
    ...
    /*Begin: Modified by zhongji for AHONG-ADD-CODE-FOR-PICK_CONTACT-EV23 2012/06/26*/
    private Cursor getboxMessages(String[] projection, String selection,
            String sortOrder){
        String unionQuery = buildBoxMessageQuery(projection, selection, sortOrder);
        return mOpenHelper.getReadableDatabase().rawQuery(unionQuery, EMPTY_STRING_ARRAY);
    }

    private static String buildBoxMessageQuery(String[] projection, String selection, String sortOrder){
        ...
        String finalMmsSelection = concatSelections(
                selection, "msg_box = 1 or msg_box = 2");

        String mmsSubQuery = mmsQueryBuilder.buildUnionSubQuery(
                MmsSms.TYPE_DISCRIMINATOR_COLUMN, innerMmsProjection,
                columnsPresentInTable, 0, "mms",
                concatSelections(finalMmsSelection, MMS_TRADICTION_CONSTRAINT),
                null, null);

        String finalSmsSelection;

        finalSmsSelection = concatSelections(selection, "type = 1 or type = 2");
        ...
        String unionQuery = unionQueryBuilder.buildUnionQuery(
                ...
                handleTradictionNullSortOrder(sortOrder), null);
    ...
    /*Begin: Modified by vicent for MSG_TRADICTION 2012/06/09*/
    private Cursor getTradictionboxMessages(
            String msg_box, String[] projection, String selection,
            String sortOrder) {
        try {
            Long.parseLong(msg_box);
        } catch (NumberFormatException exception) {
            Log.e(LOG_TAG, "Thread ID must be a Long.");
            return null;
        }

        String unionQuery = buildBoxQuery(projection, msg_box, selection, sortOrder);

        return mOpenHelper.getReadableDatabase().rawQuery(unionQuery, EMPTY_STRING_ARRAY);
    }

    private static String buildBoxQuery(String[] projection,
            String msg_box, String selection, String sortOrder) {
        String[] mmsProjection = createMmsProjection(projection);
        ...
        String finalMmsSelection = concatSelections(
                selection, Mms.MESSAGE_BOX + " = " + msg_box);
        ...
        String finalSmsSelection;
        if(msg_box.equals("4")) {
            finalSmsSelection = concatSelections(
                    selection, "type between 4 and 6");
        } else {
            finalSmsSelection = concatSelections(
                    selection, "type = " + msg_box);
        }
    ...
    private static String handleTradictionNullSortOrder (String sortOrder) {
        return sortOrder == null ? "normalized_date DESC" : sortOrder;
    }
    /*End: Modified by vicent for MSG_TRADICTION 2012/06/09*/
    ...
    private Cursor getMessagesByPhoneNumber(
        ...
        mmsQueryBuilder.setTables(
                ...
                //AHONG-DEL-CODE-FOR-COMPOSE_MSG-MULTIDELETE-EV23 zhongji@2012/05/26 DEL START
                //", (SELECT _id AS address_id " +
                //AHONG-DEL-CODE-FOR-COMPOSE_MSG-MULTIDELETE-EV23 zhongji@2012/05/26 DEL END
                /*Begin: Modified by zhongji for AHONG-ADD-CODE-FOR-PHONY-EV23 2012/05/25*/
                ", (SELECT msg_id AS address_id " +
                /*End: Modified by zhongji for AHONG-ADD-CODE-FOR-PHONY-EV23 2012/05/25*/
